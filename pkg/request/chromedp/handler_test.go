...
var (
	requestURL          *string = flag.String("url", "", "a url to request")
	remoteAllocatorAddr *string = flag.String("remote-allocator-addr", "", "a chromedp remote allocator address")
)
...
func TestRemoteHandler_Get(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()
	...
	h, err := chromedp.NewRemoteHandler(*remoteAllocatorAddr)
	if !assert.NoError(t, err) {
		return
	}
	defer h.Shutdown(context.Background())
